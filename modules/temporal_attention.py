...
class TemporalAttentionLayer2(torch.nn.Module):
  ...
  def __init__(self, n_node_features, n_neighbors_features, n_edge_features, time_dim,
    ...
    self.query_dim = n_node_features + time_dim # 128+128=256
    self.key_dim = n_node_features + time_dim + n_edge_features # 128+128+128=384

    self.out_dim = output_dimension # 128
    self.d_k = self.out_dim // self.num_heads
    self.scale = self.d_k ** (-0.5)
    self.q_linears = torch.nn.Sequential( torch.nn.Linear(self.query_dim, self.out_dim), torch.nn.ReLU()) # 256->128
    self.k_linears =  torch.nn.Sequential(torch.nn.Linear(self.key_dim, self.out_dim), torch.nn.ReLU()) # 384->128
    self.v_linears = torch.nn.Linear(self.key_dim, self.out_dim) # 384->128
